...
fn main() {
    ...
    let root5 = array_to_tree(
        &[
            Some(5), Some(4), Some(8), Some(11), None, Some(13), Some(4),
            Some(7), Some(2), None, None, None, None, None, Some(1),
        ],
        0,
    );
    ...
    let root6 = array_to_tree(
        &[
            Some(100), Some(50), Some(600), Some(45),
            Some(55), Some(500), Some(1000),
        ],
        0,
    );
    ...
    std::process::exit(
        run_tests!(&test_cases, |input| has_path_sum(input.0, input.1))
    );
}