fn main() {
    ...
    let g6 = (4, vec![vec![1, 2, 1], vec![2, 3, 1], vec![3, 4, 1], vec![4, 1, 10]]);
    let g7 = (4, vec![
        vec![1, 2, 5],
        vec![1, 3, 6],
        vec![1, 4, 4],
    ...
    let g9 = (5, vec![vec![1, 2, 1], vec![1, 3, 1], vec![1, 4, 1], vec![1, 5, 1]]);
    let g10 = (6, vec![
        vec![1, 2, 1],
        vec![2, 3, 1],
        vec![3, 4, 1],
    ...
    let g11 = (4, vec![vec![1, 2, 1], vec![2, 3, 100], vec![1, 3, 2], vec![3, 4, 1]]);
    ...
    std::process::exit(
        run_tests!(&test_cases, |input| min_connection_cost(input.0, &input.1))
    );
}
